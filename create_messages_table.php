...
try {
    $pdo->exec("
        CREATE TABLE IF NOT EXISTS public.messages (
            ...
            sender_user_id   BIGINT REFERENCES public.users (user_id) ON DELETE CASCADE,
            sender_staff_id  BIGINT REFERENCES public.staff (staff_id) ON DELETE CASCADE,
            recipient_type   VARCHAR(20) NOT NULL CHECK (recipient_type IN ('user', 'staff')),
            recipient_user_id BIGINT REFERENCES public.users (user_id) ON DELETE CASCADE,
            recipient_staff_id BIGINT REFERENCES public.staff (staff_id) ON DELETE CASCADE,
